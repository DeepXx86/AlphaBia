        ...
        for n in range(num_whole_steps):
            ...
            if randomize_symmetries != 1:
                if randomize_symmetries==2:
                    # Makruk: pure x-flip only. symm=1 is flipX; symm=5 (flipX+transpose) would
                    # wrongly transpose the SQUARE 8x8 board (Jungle's board was non-square so its
                    # transpose bit was ignored). Directional pieces (Bia/Khon) forbid y-flip/transpose.
                    symm = 1 * int(rand.integers(0, 2))  # 0:no sym   1:x-flip   others not allowed
