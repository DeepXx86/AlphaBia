...
#endif
#define COMPILE_MAX_BOARD_LEN 8  // Makruk: 8x8 board

// how many stages in each move
// eg: Makruk (like chess) has 2 stages: choosing which piece (from-square),
// and where to move it (to-square).
...
// color = (player << 4) | pieceType   (see getPiece / getPiecePla / getPieceType)
static constexpr Color C_KING   = 0x1;  // Khun  
static constexpr Color C_ROOK   = 0x2;  // Ruea 
static constexpr Color C_KNIGHT = 0x3;  // Ma    
static constexpr Color C_KHON   = 0x4;  // Khon  
static constexpr Color C_MET    = 0x5;  // Met   
static constexpr Color C_BIA    = 0x6;  // Bia  
static constexpr Color C_BIAGAE = 0x7;  // promoted Bia 

static constexpr Color NUM_PIECE_TYPES = 0x7;  // King..BiaGae

// max color = (P_WHITE<<4)|C_BIAGAE = 0x27 < 0x30, so the zobrist tables stay correct.
...
struct Board
{
  ...
  static constexpr int DEFAULT_LEN_X = 8; //Default edge length for board if unspecified
  static constexpr int DEFAULT_LEN_Y = 8; //Default edge length for board if unspecified
