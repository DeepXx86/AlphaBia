...
void Board::init(int xS, int yS)
{
  ...
  if(xS != 8 || yS != 8)
    throw StringError("Board::init - invalid board size (Makruk requires 8x8)");
  ...
  // The engine's first mover is P_BLACK, placed on the low-y ranks (advancing toward +y);
  if(x_size == 8 && y_size == 8) {
    const int blackBack = 0;            // P_BLACK back rank
    const int blackPawn = 2;            // P_BLACK Bia rank
    const int whitePawn = y_size - 3;   // P_WHITE Bia rank (=5)
    const int whiteBack = y_size - 1;   // P_WHITE back rank (=7)

    // Non-royal back-rank pieces (symmetric across files): R N S . . S N R
    setStone(Location::getLoc(0, blackBack, x_size), getPiece(P_BLACK, C_ROOK));
    setStone(Location::getLoc(7, blackBack, x_size), getPiece(P_BLACK, C_ROOK));
    setStone(Location::getLoc(1, blackBack, x_size), getPiece(P_BLACK, C_KNIGHT));
    setStone(Location::getLoc(6, blackBack, x_size), getPiece(P_BLACK, C_KNIGHT));
    setStone(Location::getLoc(2, blackBack, x_size), getPiece(P_BLACK, C_KHON));
    setStone(Location::getLoc(5, blackBack, x_size), getPiece(P_BLACK, C_KHON));

    setStone(Location::getLoc(3, blackBack, x_size), getPiece(P_BLACK, C_KING));
    setStone(Location::getLoc(4, blackBack, x_size), getPiece(P_BLACK, C_MET));

    setStone(Location::getLoc(0, whiteBack, x_size), getPiece(P_WHITE, C_ROOK));
    setStone(Location::getLoc(7, whiteBack, x_size), getPiece(P_WHITE, C_ROOK));
    setStone(Location::getLoc(1, whiteBack, x_size), getPiece(P_WHITE, C_KNIGHT));
    setStone(Location::getLoc(6, whiteBack, x_size), getPiece(P_WHITE, C_KNIGHT));
    setStone(Location::getLoc(2, whiteBack, x_size), getPiece(P_WHITE, C_KHON));
    setStone(Location::getLoc(5, whiteBack, x_size), getPiece(P_WHITE, C_KHON));
    
    setStone(Location::getLoc(4, whiteBack, x_size), getPiece(P_WHITE, C_KING));
    setStone(Location::getLoc(3, whiteBack, x_size), getPiece(P_WHITE, C_MET));

    for(int x = 0; x < x_size; x++) {
      setStone(Location::getLoc(x, blackPawn, x_size), getPiece(P_BLACK, C_BIA));
      setStone(Location::getLoc(x, whitePawn, x_size), getPiece(P_WHITE, C_BIA));
    }
  }
}
...
int Board::numPlaStonesOnBoard(Player pla) const {
  ...
  for(int y = 0; y < y_size; y++) {
    for(int x = 0; x < x_size; x++) {
      Loc loc = Location::getLoc(x,y,x_size);
      if(colors[loc] != C_EMPTY && getPiecePla(colors[loc]) == pla)
        num += 1;
    }
  }
  return num;
  ...
  else if(stage == 1)  //place
  {
    ...
    if(isOnBoard(loc)) {
      ...
      if(isOnBoard(chosenLoc)) {
        ...
        Color movingPiece = colors[chosenLoc];
        setStone(loc, movingPiece);
        setStone(chosenLoc, C_EMPTY);
        // Makruk promotion: a Bia reaching its promotion rank becomes a Bia Gae (moves as a Met).
        // Deterministic and forced - no extra stage or promotion flag is needed.
        if(getPieceType(movingPiece) == C_BIA &&
           Location::getY(loc, x_size) == GameLogic::promotionRankY(pla, y_size))
          setStone(loc, getPiece(pla, C_BIAGAE));
      }
    }
...
char PlayerIO::colorToChar(Color c)
{
  ...
  switch(c) {
    case C_KING:   t = 'k'; break;  // Khun
    case C_ROOK:   t = 'r'; break;  // Ruea
    case C_KNIGHT: t = 'n'; break;  // Ma
    case C_KHON:   t = 's'; break;  // Khon
    case C_MET:    t = 'm'; break;  // Met
    case C_BIA:    t = 'p'; break;  // Bia
    case C_BIAGAE: t = 'q'; break;  // promoted Bia (Bia Gae)
  default:  return '#';
  }
  if(p == C_BLACK)
    t += ('A' - 'a');
...
Color PlayerIO::charToColor(char c) {
  ...
  if(c >= 'A' && c <= 'Z')
    c += ('a' - 'A');
  switch(c) {
    case 'k': t = C_KING; break;
    case 'r': t = C_ROOK; break;
    case 'n': t = C_KNIGHT; break;
    case 's': t = C_KHON; break;
    case 'm': t = C_MET; break;
    case 'p': t = C_BIA; break;
    case 'q': t = C_BIAGAE; break;
