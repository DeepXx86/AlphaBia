...
void NNInputs::fillRowV7(
  ...
  // Makruk: king-in-check signals (computed once), written into planes 17/18 below.
  bool myInCheck = GameLogic::isInCheck(board, pla);
  bool oppInCheck = GameLogic::isInCheck(board, opp);
  Loc myKingLoc = GameLogic::findKing(board, pla);
  Loc oppKingLoc = GameLogic::findKing(board, opp);
  ...
  for(int y = 0; y<ySize; y++) {
    for(int x = 0; x<xSize; x++) {
      ...
      //17,18: a king currently in check (side-to-move's king / opponent's king).
      //Repurposed from the Jungle home/trap/river planes, which Makruk does not use;
      //planes 19,20,21 are left as zeros (reserved).
      if(loc == myKingLoc && myInCheck)
        setRowBin(rowBin, pos, 17, 1.0f, posStride, featureStride);
      if(loc == oppKingLoc && oppInCheck)
  ...
  for(int y = 0; y < ySize; y++) {
    for(int x = 0; x < xSize; x++) {
      ...
      if(stone != C_EMPTY) {
        ...
        // Features 1~16 stones (Makruk uses types 1..7 -> mine 1-7, opp 9-15)
        setRowBin(rowBin, pos, idx, 1.0f, posStride, featureStride);
      }

      // 17,18: a king currently in check (side-to-move's / opponent's). Repurposed from
      // the Jungle home/trap/river planes; 19,20,21 are reserved zeros for Makruk.
      if(loc == myKingLoc && myInCheck)
        setRowBin(rowBin, pos, 17, 1.0f, posStride, featureStride);
      if(loc == oppKingLoc && oppInCheck)
        setRowBin(rowBin, pos, 18, 1.0f, posStride, featureStride);
